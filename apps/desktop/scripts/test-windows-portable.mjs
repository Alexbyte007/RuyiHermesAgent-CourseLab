...
function main() {
  ...
  const expectedApplicationMachine = process.arch === 'arm64' ? 0xaa64 : 0x8664
  const applicationMachine = readPeMachine(unpacked.binary)
  if (applicationMachine !== expectedApplicationMachine) {
    throw new Error(
      `portable application has the wrong PE architecture for ${process.arch}: ` +
        `expected 0x${expectedApplicationMachine.toString(16)}, got 0x${applicationMachine.toString(16)}`
    )
  }

  const launcherMachine = readPeMachine(artifact)
  const allowedLauncherMachines =
    process.arch === 'x64'
      ? new Set([0x014c, 0x8664])
      : new Set([expectedApplicationMachine])
  if (!allowedLauncherMachines.has(launcherMachine)) {
    throw new Error(
      `portable launcher has the wrong PE architecture for ${process.arch}: ` +
        `got 0x${launcherMachine.toString(16)}`
    )
  }
